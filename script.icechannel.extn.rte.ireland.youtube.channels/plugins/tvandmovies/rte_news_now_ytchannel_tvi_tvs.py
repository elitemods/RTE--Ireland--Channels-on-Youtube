...
class rte_news_now_ytchannel(TVShowIndexer, TVShowSource):
    implements = [TVShowIndexer, TVShowSource]

    name = 'Youtube: RTE News Now'
    display_name = 'Youtube: RTE News Now'
    
    img = 'http://yt3.ggpht.com/-nsZYzcdin2g/AAAAAAAAAAI/AAAAAAAAAAA/BsOo0PvZct8/s256-c-k-no/photo.jpg'
    ...
    def GetSection(self, indexer, section, url, type, list, page='', total_pages='', sort_by='', sort_order=''):
        if section == 'main':
            self.AddSection(list, indexer, 'uploads', 'Uploads', 'http://gdata.youtube.com/feeds/api/users/RTENewsNow/uploads?&v=2&max-results=50&alt=jsonc', indexer)
            self.AddSection(list, indexer, 'playlists', 'Playlists', 'http://gdata.youtube.com/feeds/api/users/RTENewsNow/playlists?&v=2&max-results=50&alt=jsonc', indexer)
        else:
